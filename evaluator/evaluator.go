...
var (
	...
	typeMap = map[object.Type]string{
...
func Eval(node ast.Node, env *object.Environment) object.Object {
	...
	case *ast.ErrorLiteral:
		return &object.Error{Message: node.Value}
	...
	case *ast.CallExpression:
		...
		case *object.Function:
			...
			if typeMap[result.Type()] == fn.ReturnType.Token.Literal {
				...
				if result.Type() == object.ClassObj {
...
func evalHashIndexExpression(hash, index object.Object) object.Object {
	...
	if !ok {
		return newError("key error: %s not found", index.Inspect())
	}
...
func evalHashLiteral(node *ast.HashLiteral, env *object.Environment) object.Object {
	...
	for keyNode, valueNode := range node.Pairs {
		...
		if t == "" {
			...
			if t != typeMap[key.Type()] {
				return newError("trying to assign %s to map of %s", typeMap[key.Type()], t)
			}
		}
